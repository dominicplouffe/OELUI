...
const Node = ({ pongKey, api_url }) => {
  return (
    <Row>
      <Col xs={12}>
        <pre className="code p-4" style={{ lineHeight: "25px" }}>
          <span className="text-success">
            {"// "}# Example to send a start and end request to onErrorLog using Node.js
          </span>
          <br />
          <span className="text-danger">const</span> request = <span className="text-success">require</span>
          ...
          <br />
          {"   "} <span className="text-danger">if</span> (err) {`{`} <span className="text-danger">return </span>
          console.
      ...
      <Col xs={12}>
        <small>
          Note: You need to install the request library: <strong>npm install request</strong>
        </small>
      </Col>
    </Row>
  );
