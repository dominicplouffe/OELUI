...
const Heartbeat = (props) => {
  ...
  const fetchSummary = async (skip = false) => {
    ...
    if (!skip) {
      ...
      if (error) {
        alert("Something went wrong, we cannot find your heartbeat");
      }
    }
  };
  useEffect(() => {
  ...
  return (
    <Body title="Heartbeats" selectedMenu="heartbeat" {...props} loading={loading}>
      <Row>
          ...
          <Card.Title>
            <Row>
              <Col xs={12} lg={3} className="text-center">
                <h3>Heartbeats Summary</h3>
              </Col>
              <Col className="right-align-small-center">
                <Link to="/newheartbeat" className="btn btn-warning btn-rounded">
                  New Heartbeat
                </Link>
              </Col>
            </Row>
      ...
      {pings.map((m, i) => (
        <HeartbeatCard m={m} key={i} showSummary={true} showEdit={true} />
      ))}
          ...
          <Card.Body className="p-5">
            <Row>
              <Col className="text-center">
                ...
                />
                <h2 className="pt-3">
                  You have not added any Heartbeats to your account yet.
                </h2>
                <div className="pt-5">
                  <Link
                    to="/heartheat/0"
