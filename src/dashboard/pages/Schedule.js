...
const Schedule = (props) => {
  ...
  const fetchUsers = async () => {
    const { data = null, error = null } = await api("/api/org_user/?ordering=created_on", "GET", {});
    ...
    if (data) {
      ...
      users.sort((a, b) => (a.schedule.order > b.schedule.order ? 1 : b.schedule.order > a.schedule.order ? -1 : 0));
  ...
  const getOverrides = async (users) => {
    ...
    const { data = null, error = null } = await api(`schedule_override/?start_date__gte=${start} 00:00:00`);
  ...
  const generateCalendarEvents = (users, overrides) => {
    ...
    let calDate = new Date(start.getFullYear(), start.getMonth(), start.getDate());
  ...
  const moveUser = (from, to) => {
    if (to < 0 || to >= users.length) return;
    const updated = [...users];
  ...
  const setOverrideDate = (dt, date) => {
    ...
    if (isError) {
      setOverrideDateError("Dates must be bigger than today and end date must be larger than to the start date");
    } else {
      setOverrideDateError("");
    }
  };
  ...
  const saveOverride = async () => {
    ...
    const { data = null, error = null } = await api(`schedule_override/`, "POST", payload);
  ...
  return (
    <Body title="Schedule" selectedMenu="schedule" loading={loading} {...props}>
        ...
        <Card.Body>
          <Row>
            <Col xs={12} lg={3}>
              <Card.Title>
                <Row>
                  <Col className="fs-5">On Call Schedule</Col>
                  <Col className="text-end">
                    <Badge pill variant="light">
                      <>
                        <span>Current Week </span>
                        <span>{currentUser.role.org.week}</span>
                      </>
                    </Badge>
                  </Col>
                </Row>
              ...
              <ul style={{ listStyle: "none", padding: 0 }}>
                {users.map((user, index) => (
                  <li key={user.id} style={getItemStyle(false, null, user)}>
                    <span className="pe-3">
                      <button
                    ...
                    {user.schedule.order === user.org.week ? (
                      <span className="ps-2" role="img" aria-label="is-on-call">
                        📱
                      </span>
                    ) : null}
                  </li>
              ...
              {showSave && (
                <Row className="mt-3 mb-3">
                  <Col>
                    <Button type="custom" className="btn btn-block btn-primary" onClick={() => handleSaveSchedule()}>
                      Save Schedule
                    </Button>
                  </Col>
                </Row>
      ...
      <Modal show={showOverrideDelete} onHide={() => setShowOverrideDelete(false)} aria-labelledby="override-modal">
          ...
          {override && (
            <Table borderless size="sm">
              ...
              <tr>
                ...
                <td>
                  {users.filter((u) => u.id === override.org_user)[0].first_name}{" "}
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowOverrideDelete(false)}>
            Close
          </Button>
      ...
      <Modal show={showOverride} onHide={() => setShowOverride(false)} aria-labelledby="override-modal">
        <Modal.Header closeButton>
        ...
        <Modal.Body>
          ...
          <div className="form-group mt-3">
            <label className="form-label">Which dates do you want to override?</label>
          </div>
          <Row>
            <Col xs={12} lg={6}>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => saveOverride(false)} disabled={overrideDateError.length > 0}>
            Override
          </Button>
        </Modal.Footer>
      </Modal>
